...
class Dataset(torch.utils.data.Dataset):
    ...
    def load_mask(self, img, index):
        ...
        if self.training:
            ...
            mask_index = index
            print('+++++++++++++++')
        #print("mask index: ", self.mask_data[mask_index])
    ...
    def resize(self, img, height, width, centerCrop=True):
        ...
        #size = tuple((np.array([height, width])))
        #new_image = np.array(img.resize(size, PIL.Image.BICUBIC))
        #from skimage.transform import resize
        #new_image = resize(img, output_shape=(num_px,num_px)).reshape((1, num_px*num_px*3)).T
